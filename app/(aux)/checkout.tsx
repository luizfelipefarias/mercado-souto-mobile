...
export default function Checkout() {
  ...
  useEffect(() => {
    async function loadAddress() {
      ...
      try {
        const res = await api.get<any[]>(`/api/address/by-client/${user.id}`);
  ...
  const handleFinish = async () => {
    if (!signed || (user as any)?.isGuest) {
      Alert.alert(
        ...
        [
          ...
          { text: 'Entrar', onPress: () => router.push('/login' as any) },
        ]
      );
      return;
    }
    ...
    if (!address) {
      Alert.alert(
        ...
        [
          { text: 'Cancelar', style: 'cancel' },
          { text: 'Adicionar endereço', onPress: () => router.push('/address' as any) },
        ]
      );
      return;
    }
    ...
    try {
      ...
      clearCart();
      Alert.alert('Sucesso!', 'Pedido realizado. Enviamos os detalhes para seu e-mail.');
      router.replace('/my-purchases' as any);
    } catch (error: any) {
          ...
          <View style={styles.addressBox}>
            ...
            <View style={{ marginLeft: 10, flex: 1 }}>
              {loadingAddress ? (
                ...
                <>
                  <Text style={{ fontWeight: 'bold' }}>{address.contactName || 'Principal'}</Text>
                  <Text style={{ color: '#666' }}>{`${address.street || ''} ${address.number || ''}`.trim()}</Text>
                  <Text style={{ color: '#666', fontSize: 12 }}>{(user as any)?.name}</Text>
                </>
              ) : (
                <>
