...
import { useLocalSearchParams, useRouter, useFocusEffect } from 'expo-router'; // 🟢 Importando useFocusEffect
...
// Importa tipos de Address para tipagem correta
...
// 🟢 Novo tipo para o endereço selecionado (pode ser Address completo + dados de exibição)
...
export default function ProductDetails() {
    ...
    // 🟢 ESTADO PARA O ENDEREÇO ATIVO NA BARRA DE LOCALIZAÇÃO
    ...
    // 🟢 LÓGICA DE BUSCA DO ENDEREÇO ATIVO (para a barra de localização)
        ...
        try {
            ...
            if (isGuest) {
                ...
                if (stored) {
                    ...
                    addresses = JSON.parse(stored) as ActiveAddress[]; 
                }
            } else if (user?.id) {
                const res = await api.get<Address[]>(`/api/address/by-client/${user.id}`);
                // Mapeia para incluir campos de exibição se a API não os retornar
    ...
    // 🟢 Usa useFocusEffect para recarregar o endereço quando o usuário retorna do AddressList
    useFocusEffect(
        useCallback(() => {
            fetchActiveAddress();
        }, [fetchActiveAddress])
    ...
    const getAddressText = () => {
        ...
        if (activeAddress && activeAddress.street && activeAddress.number) {
            const streetInfo = `${activeAddress.street}, ${activeAddress.number}`;
            const cityInfo = activeAddress.city && activeAddress.state ? ` (${activeAddress.city}/${activeAddress.state})` : '';
            return `Enviar para ${user?.name?.split(' ')[0] || activeAddress.contactName || 'Você'}: ${streetInfo}`;
        }
